...
# RUNNING PARAMETERS
# ==================
log_dir="anneal_chain_logs"
state=Michigan
maps=("state_house" "state_senate")
county_weights=(0 .25 .5 .75 1)
county_sub_weights=(0 .25 .5 .75 1)
hots=(10 100 500 1000 5000)
colds=(10 100 500 1000 5000)
beta_magnitudes=(1 10 100 500)
N=100000
...
generate_file_label() {
    ...
    echo "${state// /-}"\
        ...
        "hot_${hot// /-}"\
        "cold_${cold// /-}"\
        "beta_magnitude_${beta_magnitude// /-}"\
        | tr ' ' '_'
...
for hot in "${hots[@]}"; do
for cold in "${colds[@]}"; do
for beta_magnitude in "${beta_magnitudes[@]}"; do
