...
#SBATCH --mem=18000 # how many MB of memory do we need (4GB here)
...
if [ $6 = 0 ]; then
    ...
    python MI_plots.py ${2} "vra_neutral" ${4} ${5} 2 ${7} ${8} 

elif [ $6 = 1 ]; then
    echo ${1} ${2} ${3} ${4} ${5} ${7} ${8}  --VRA_reject 
    # feed in value for theta 
    python MI_plots.py ${2} "vra_reject" ${4} ${5} 2 ${7} ${8} 
else
    #if vra_climb, submit 3 batch jobs for values of theta
    echo ${1} ${2} ${3} ${4} ${5} ${7} ${8}  --VRA_climb
    cd /cluster/tufts/mggg/cdonna01/MI_effective/full_state_scripts
    generate_file_label() {
            local state="$1"
            local map="$2"
            local N="$3"
    ...
    log_file="figure_logs/${file_label}.log"
    sbatch --job-name="vra climb" --output="${log_file}" \
        --error="${log_file}" vra_climb_figures.sh ${1} ${2} ${3} ${4} ${5} ${7} ${8} $log_file 10
    ...
    log_file="figure_logs/${file_label}.log"
    sbatch --job-name="vra climb" --output="${log_file}" \
        --error="${log_file}" vra_climb_figures.sh ${1} ${2} ${3} ${4} ${5} ${7} ${8} $log_file 50
    ...
    log_file="figure_logs/${file_label}.log"
    sbatch --job-name="vra climb" --output="${log_file}" \
        --error="${log_file}" vra_climb_figures.sh ${1} ${2} ${3} ${4} ${5} ${7} ${8} $log_file 100
fi
