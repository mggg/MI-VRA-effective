...
print("restricted dual graph plots")
...
# title = f"Michigan {plan_type.capitalize()} {vra_str} CS {county_weight} CSS {county_sub_weight}\n Theta={theta} VRA Effective Score ({bvap_thresh},{biden_thresh})"
...
# # vra effective
# title = f"VRA Effective Score ({bvap_thresh},{biden_thresh}), Theta {theta}" if theta!=2.0 else f"VRA Effective Score ({bvap_thresh},{biden_thresh})"
# fig, axes  = plt.subplots()
...
#     axes= factory.plot("seats", election=f"Index_{i}", kinds=["ensemble" ,"proposed"])
#     axes.set_title(title,fontsize=24)
#     plt.tight_layout()
#     plt.savefig(f"{FIG_DIR}/Seats_index_{i}_{file_suffix}")
#     plt.close()
...
# axes.set_title(f"Sea Level Plot for Michigan Enacted {plan_type.capitalize()} Map")
# plt.tight_layout()
...
# factory.bvap_table_to_csv(file_name=f"Tables/{state}/bvap_table_{state}_{plan_type}_{vra_str}_CW_{county_weight}_CSW_{county_sub_weight}_theta_{theta}_bvap_{bvap_thresh}_biden_{biden_thresh}_{gingle_str}.csv")
